...
class Node:
    ...
    def find(self, by: By | str, value: str, parent_element: WebElement = None, wait: int = None, timeout: int = None):
        '''
...
class BrowserManager:                                                                                                                                                                              # Anh em cứ yên tâm chạy nhé, không có mã độc đâu! :D Chỉ giả vờ đánh lừa người ngoài chút thôi! 🚀
    ...
    def _browser(self, profile_name: str, proxy_info: str = None) -> webdriver.Chrome:
        '''
        ...
        # Start Thử nghiệm option tăng hiệu suất chrome
        chrome_options.add_argument("--disable-gpu")  # Vô hiệu hóa GPU
        chrome_options.add_argument("--no-sandbox")  # Không sử dụng sandbox
        chrome_options.add_argument("--disable-dev-shm-usage")  # Tránh lỗi bộ nhớ chia sẻ nhỏ
        chrome_options.add_argument("--disable-background-timer-throttling")  # Ngăn Chrome làm chậm các tab nền
        chrome_options.add_argument("--disable-backgrounding-occluded-windows")  # Ngăn Chrome làm chậm cửa sổ bị ẩn
        chrome_options.add_argument("--disable-renderer-backgrounding")  # Luôn ưu tiên tài nguyên cho Chrome
        chrome_options.add_argument("--disable-popup-blocking")  # Tránh các cảnh báo popup gây chậm
        # End Thử nghiệm option tăng hiệu suất chrome        

        # start demo loại bỏ not secure, nhưng chưa đc
        chrome_options.add_argument('--ignore-certificate-errors')
        chrome_options.add_argument("--allow-running-insecure-content") #// Chạy nội dung không an toàn
    ...
    def run_terminal(self, profiles: list[dict], auto: bool = False, max_concurrent_profiles: int = 4, headless: bool = False):
        ...
        while is_run:
            if user_data_dir.exists() and user_data_dir.is_dir():
                user_data_profiles = [folder.name for folder in user_data_dir.iterdir() if folder.is_dir()]
            ...
            if choice in ('1', '2', '3'):
                ...
                if not auto:
                    ...
                    for idx, profile in enumerate(profile_list, start=1):
                        print(f"{idx}. {profile['profile_name'] if choice in ('1', '2') else profile}")
...
if __name__ == '__main__':
    ...
    for line in data:
        ...
        profile_name, email, *_ = (parts + [None] * num_parts)[:num_parts]
